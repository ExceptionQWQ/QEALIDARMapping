...
cv::Mat GetCloudImage()
{
    ...
    for (int i = 0; i < 1200; ++i) {
        ...
        double x = robotInfo.xPos + lidarPointData[i].distance * cos((PI / 180 * lidarPointData[i].angle) + 0) * 0.4;
        double y = 1500 - (robotInfo.yPos + lidarPointData[i].distance * sin((PI / 180 * lidarPointData[i].angle) + 0) * 0.4);
        DrawPoint(image, x, y);
    }
    cv::Mat ret;
